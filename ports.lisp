...
(fn request-port (port fun)
  (aadjoin! port fun *requested-ports* :test #'eq))

(fn (= group-port) (port group)
...
(fn generate-ports ()
  (= *ports* (@ [. _. (funcall ._)] *requested-ports*)))
...
(fn component-port (x)
  (group-port (action-group (component-action x))))

(fn components-w/o-port (port)
