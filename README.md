...
saves it as 'compiled/index.php'.  Create 'toplevel.lisp' with this:
...
Parameters give templates their dynamic content.  They're passed to template
functions in the form of an associative list.  Inside the template function
PARAM gets you a particular parameter.  The LML expressions are
BACKQUOTEd automatically, so you must use QUASIQUOTE to insert values.
...
Form and database records come as associative lists as well.  Our first, simple
...
This assumes that the script is in the root directory of your web server.
...
phitamine processes an URL directory by directory.  When a directoriy's name is
associated with a handler (called `action') that handler is called, so it can
configure the application or claim a part in the page generated in TPL-MAIN
called in your 'toplevel.lisp'.

If phitamine hits a component for which no action was defined, it issues
the good old mood–shifting ERROR 404 PAGE NOT FOUND.
...
actions are packed into groups which can be associated with a port.  If you
